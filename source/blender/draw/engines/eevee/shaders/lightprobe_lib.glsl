...
/*   for (int i = 1; i < MAX_PROBE && i < prbNumRenderCube && spec_accum.a < 0.999; i++) {
  ...
  /* World Specular */
  if (spec_accum.a < 0.999) {
    vec3 spec = textureLod_cubemapArray(probeCubes, vec4(N, 0.0), prbLodCubeMax).rgb; //vec3 spec = textureLod_cubemapArray(probeCubes, vec4(N, ssrDiffuseDebugA), prbLodCubeMax).rgb;
    accumulate_light(spec, 1.0, spec_accum);
  }
}
