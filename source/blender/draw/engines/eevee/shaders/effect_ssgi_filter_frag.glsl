...
vec3 doAtrous (in vec2 uv, sampler2D source)
{
  ...
  float stepwidth = ssrDiffuseFsize / mix(1.0, 3.0, sizevariance);
  /* input block end */
  ...
  //AO weight
  float aoweight = 0.1 * ssrDiffuseFaoweight;
 ...
 void main()
 {
  ...
  if (samplePoolOffset > ssrDiffuseFsamples) {
    filtered = vec4(doAtrous(uvcoordsvar.xy, ssgiFilterInput), 1.0);
  }
  ssgiFilterSecInput = filtered;
 }