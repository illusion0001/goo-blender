...
void blo_do_versions_300(FileData *fd, Library * /*lib*/, Main *bmain)
{
  ...
  if (!DNA_struct_elem_find(fd->filesdna, "SceneEEVEE", "float", "diffuse_intensity")) {
    LISTBASE_FOREACH (Scene *, scene, &bmain->scenes) {
      if (scene->eevee.ssr_diffuse_versioning < 1.160f) {
        scene->eevee.ssr_diffuse_versioning = 1.161f;
        ...
        scene->eevee.ssr_diffuse_ao_limit = 0.25f;
        ...
        scene->eevee.ssr_diffuse_fsize = 16.0f;
